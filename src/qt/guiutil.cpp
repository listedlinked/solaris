...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tesla address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Tesla: URI
...
{
    // Convert tesla:// to tesla:
    //
    //    Cannot handle this later, because tesla:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open tesla.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Tesla.lnk";
}
...
{
    return GetAutostartDir() / "tesla.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a tesla.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Tesla\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the tesla app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add tesla app to startup item list
